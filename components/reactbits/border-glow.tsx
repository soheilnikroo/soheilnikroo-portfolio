...
// Adapted from React Bits (https://reactbits.dev) — "BorderGlow". Named export, TS.
...
function buildMeshGradients(colors: string[]): string[] {
  ...
  for (let i = 0; i < 7; i++) {
    const c = colors[Math.min(COLOR_MAP[i] ?? 0, colors.length - 1)] ?? "#e6c463";
    gradients.push(`radial-gradient(at ${GRADIENT_POSITIONS[i]}, ${c} 0px, transparent 50%)`);
  }
  gradients.push(`linear-gradient(${colors[0] ?? "#e6c463"} 0 100%)`);
  return gradients;
}
