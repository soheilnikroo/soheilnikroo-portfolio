...
/**
 * The player character as a set of named clips, each with east/west frames. The
 * engine never animates the character on a timer — a chapter chooses the clip and
 * the exact frame index from scroll progress, so the pose is a pure function of
 * the timeline and rewinds exactly.
 */
...
export interface CharacterDraw {
  /** Screen x of the character's horizontal centre. */
  readonly x: number;
  /** Screen y of the character's feet (baseline). */
  readonly baseline: number;
  readonly scale: number;
  readonly clip: ClipName;
  readonly frame: number;
  readonly dir: Direction;
  /** Optional soft contact shadow. Defaults to true. */
  readonly shadow?: boolean;
  /** Vertical bob in virtual px (idle sway, landing bounce). */
  readonly bob?: number;
}
...
export interface CharacterLoadOptions {
  /** Clips required before the first frame (remaining clips hydrate in the background). */
  readonly priorityClips?: readonly ClipName[];
}

/**
 * Fraction of the (92px) frame, measured from the top, at which the figure's feet
 * actually sit — the rest of the frame below is transparent padding. Measured from
 * the sprite alpha (bottom non-transparent row ≈ 72/92). Aligning this row to the
 * baseline keeps the character planted on the ground instead of floating above it.
 */
...
export class Character {
  ...
  draw(surface: RenderSurface, opts: CharacterDraw): void {
    ...
    // Place the figure's feet (FOOT_FRAC of the frame) exactly on the baseline so
    // it stands on the ground; the transparent padding below just hangs past it.
